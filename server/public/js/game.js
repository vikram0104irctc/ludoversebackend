...
class Game {
  constructor(players, computerOpponentLevels = [], tokenPositons = undefined) {
    ...
    for (let player of this.players) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        document
          .querySelector(`#${player}-token-${i + 1}__pawn-shape`)
        ...
        if (position === -6) {
          token.setAttribute(
            "translation",
            `${startPosition[1]} 1 ${startPosition[0]}`
          );
        } else {
    ...
    for (let player of allPossiblePlayers) {
      if (
        this.players.findIndex(
          (possiblePlayer) => possiblePlayer === player
        ) !== -1
      ) {
        for (let i = 0; i < 4; i++) {
          const id = `${player}-token-${i + 1}`;
    ...
    for (let i = 0; i < computerOpponentLevels.length; i++) {
      ...
      this.computerOpponents[index] = new (
        computerOpponentLevels[i] === "Hard"
          ? MonteCarloOpponent
          : ComputerOpponent
      )(this, this.players[index], index);
      document.querySelector(
        `.scoreboard > .${this.players[index]} .dot`
      ).dataset.ai = "true";
    }
   ...
   * */ checkPossibleMoves() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (newPosition >= 0 && newPosition < playerPaths[player].length) {
        ...
        document
          .querySelector(`#${player}-token-${i + 1}__pawn-material`)
          .setAttribute("emissiveColor", "0.5 0.5 0.5");
      }
    }
  ...
  nextPlayer() {
    ...
    root.style.setProperty(
      "--background-color",
      `var(--${this.players[this.currentPlayerIndex]}-bg)`
    );
    root.style.setProperty(
      "--text-color",
      `var(--${this.players[this.currentPlayerIndex]}-text)`
    );

    viewPoints[this.players[this.currentPlayerIndex]].setAttribute(
      "set_bind",
      "true"
    );
   ...
   * */ moveToken(player, i) {
    ...
    if (
      safeTiles.findIndex((safeTile) => arraysEqual(safeTile, newTile)) === -1
    ) {
      for (let otherPlayer of this.players) {
        ...
        for (let i = 0; i < 4; i++) {
          const otherTokenTile =
            playerPaths[otherPlayer][this.tokenPositons[otherPlayer][i]];
        ...
        if (otherTilesFoundIndices.length === 1) {
          ...
          setTimeout(() => {
            ...
            const timeSensor = document.getElementById(
              `time-${otherPlayer}-${otherTileIndex + 1}`
            );
            const positionInterpolator = document.getElementById(
              `move-${otherPlayer}-${otherTileIndex + 1}`
            );
            ...
            tokens[otherPlayer][otherTileIndex].setAttribute(
              "translation",
              `${startPosition[1]} 1 ${startPosition[0]}`
            );
    ...
    for (
      let positionIndex = Math.max(oldPosition, -1);
      positionIndex < newPosition;
      positionIndex++
    ) {
      const position =
        positionIndex == -1 ? oldTile : playerPaths[player][positionIndex];
      const nextPosition = playerPaths[player][positionIndex + 1];

      keyValue += `  ${(position[1] + nextPosition[1]) / 2} 2 ${
        (position[0] + nextPosition[0]) / 2
      }  ${nextPosition[1]} 1 ${nextPosition[0]}  `;
      movesCount += 2;
    }
    ...
    setTimeout(() => {
      tokens[player][i].setAttribute(
        "translation",
        `${newPosition[1]} 1 ${newPosition[0]}`
      );
      ...
      if (
        this.lastRolledValue === 6 &&
        this.isCurrentPlayerComputerOpponent()
      ) {
        this.moveComputerOpponent(this.currentPlayerIndex);
      }
  ...
  getFreeStartPosition(player) {
    ...
    for (let position of startPositions[player]) {
      if (
        takenStartPositions.findIndex((takenPosition) =>
          arraysEqual(takenPosition, position)
        ) === -1
      ) {
        return position;
      }
  ...
  moveComputerOpponent(index) {
    setTimeout(() => (infoDiv.innerText = "Computer's turn"), 1000);

    setTimeout(() => this.computerOpponents[index].makeMove(), 2000);
  }

  resetRolledValue() {
    this.lastRolledValue = 0;
    infoDiv.innerText = this.isCurrentPlayerComputerOpponent()
      ? "Computer's turn"
      : "Roll the dice!";
  }
  ...
  clearPossibleMoves() {
    for (let player of this.players) {
      for (let i = 0; i < 4; i++) {
        document
          .querySelector(`#${player}-token-${i + 1}__pawn-material`)
          .setAttribute("emissiveColor", "0 0 0");
      }
    }
  }
    ...
    for (let i = 0; i < tokenCount; i++) {
      if (allTokensOnTile[i]) {
        const xOffset =
          tokenCount == 1
            ? 0
            : r * Math.cos((Math.PI / 180) * i * (360 / tokenCount));
        const yOffset =
          tokenCount == 1
            ? 0
            : r * Math.sin((Math.PI / 180) * i * (360 / tokenCount));

        allTokensOnTile[i].setAttribute(
          "translation",
          `${tile[1] + yOffset} 1 ${tile[0] + xOffset}`
        );
      }
    }
  }
...
function onRollButtonClick() {
  if (game.isCurrentPlayerComputerOpponent()){
    return;
  }
